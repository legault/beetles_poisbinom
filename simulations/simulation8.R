...
d1 <- rbinom(n = sims, size = sim.stor[, 1],
             prob = model8(sim.stor[, 6], param8$p0, param8$b4)) # dispersal patch1
d2 <- rbinom(n = sims, size = sim.stor[, 2],
             prob = model8(sim.stor[, 5], param8$p0, param8$b4)) # dispersal patch2
...
# Save results
write.table(sim.stor, file = "../data/simulation8.csv", sep = ",", col.names = TRUE, row.names = FALSE)
