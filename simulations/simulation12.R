...
d1 <- rbinom(n = sims, size = sim.stor[, 1],
             prob = model12(sim.stor[, 1], sim.stor[, 2], sim.stor[, 5], param$p0, param$b1, param$b2, param$b3)) # dispersal patch1
d2 <- rbinom(n = sims, size = sim.stor[, 2],
             prob = model12(sim.stor[, 2], sim.stor[, 1], sim.stor[, 6], param$p0, param$b1, param$b2, param$b3)) # dispersal patch2
...
# Save results
write.table(sim.stor, file = "../data/simulation12.csv", sep = ",", col.names = TRUE, row.names = FALSE)
