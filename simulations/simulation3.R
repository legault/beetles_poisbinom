...
d1 <- rbinom(n = sims, size = sim.stor[, 1],
             prob = model3(sim.stor[, 1], sim.stor[, 5], param$p0, param$b1, param$b3)) # dispersal patch1
d2 <- rbinom(n = sims, size = sim.stor[, 2],
             prob = model3(sim.stor[, 2], sim.stor[, 6], param$p0, param$b1, param$b3)) # dispersal patch2
