...
d1 <- rbinom(n = sims, size = sim.stor[, 1],
             prob = model9(sim.stor[, 1], sim.stor[, 6], param9$p0, param9$b1, param9$b4)) # dispersal patch1
d2 <- rbinom(n = sims, size = sim.stor[, 2],
             prob = model9(sim.stor[, 2], sim.stor[, 5], param9$p0, param9$b1, param9$b4)) # dispersal patch2
...
# Save results
write.table(sim.stor, file = "../data/simulation9.csv", sep = ",", col.names = TRUE, row.names = FALSE)
