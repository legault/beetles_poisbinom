...
d1 <- rbinom(n = sims, size = sim.stor[, 1],
             prob = model15(sim.stor[, 2], sim.stor[, 5], sim.stor[, 6], param$p0, param$b2, param$b3, param$b4)) # dispersal patch1
d2 <- rbinom(n = sims, size = sim.stor[, 2],
             prob = model15(sim.stor[, 1], sim.stor[, 6], sim.stor[, 5], param$p0, param$b2, param$b3, param$b4)) # dispersal patch2
...
# Save results
write.table(sim.stor, file = "../data/simulation15.csv", sep = ",", col.names = TRUE, row.names = FALSE)
